"""
    生成 METR-LA 或 PEMS-BAY 数据集每个传感器的所有预测点对【其他传感器】【最近时刻】的检测结果
    `indices`: 所有的预测结果范围
...
if __name__ == '__main__':
    ...
    res_dir = f'analyse_testing_result3/{args.dataset}/{args.output_window}_{args.input_dim}'
    ensure_dir(res_dir)
    ...
    for nn in range(sensors):
        ...
        for index in indices:
            ...
            for test_nn in range(sensors):
                ...
                if test_nn == nn:
                    if args.input_dim == -1:
                        tot_data.append(np.mean(read_data[11, :]))
                    else:
                        tot_data.append(read_data[11, args.input_dim])
                else:
                    if args.input_dim == -1:
                        tot_data.append(np.mean(read_data[0, :]))
                    else:
                        tot_data.append(read_data[0, args.input_dim])

        tot_data = np.array(tot_data).reshape((len(indices), sensors))   # (len(indices), 207)
        df = pd.DataFrame(tot_data, index=indices)
        df.to_csv(f'{res_dir}/{nn}.csv')
