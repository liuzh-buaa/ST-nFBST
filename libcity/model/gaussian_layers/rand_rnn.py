...
from libcity.model.gaussian_layers.rand_rnn_cells import RandLSTMCell, RandGRUCell, RandRNNCell
...
class RandRNNLayer(nn.Module):
    def __init__(self, cell, *cell_args):
        super(RandRNNLayer, self).__init__()
    ...
    def set_shared_eps(self):
        self.cell.set_shared_eps()

    def clear_shared_eps(self):
        self.cell.clear_shared_eps()

    def get_kl_sum(self):
        return self.cell.get_kl_sum()
...
class RandRNNBase(nn.Module):
    ...
    def __init__(self, mode: str, input_size: int, hidden_size: int,
                 ...
                 dropout: float = 0., bidirectional: bool = False, proj_size: int = 0, random_bias: bool = True,
                 kl_bias: bool = True, sigma_pi: float = 1.0, sigma_start: float = 1.0) -> None:
        ...
        if mode == 'LSTM':
            self.layers = init_stacked_layers(self.num_layers, RandRNNLayer,
                                              [RandLSTMCell, input_size, hidden_size, self.bias, self.random_bias,
                                               self.kl_bias, self.sigma_pi, sigma_start],
                                              [RandLSTMCell, hidden_size, hidden_size, self.bias, self.random_bias,
                                               self.kl_bias, self.sigma_pi, sigma_start])
        elif mode == 'GRU':
            self.layers = init_stacked_layers(self.num_layers, RandRNNLayer,
                                              [RandGRUCell, input_size, hidden_size, self.bias, self.random_bias,
                                               self.kl_bias, sigma_pi, sigma_start],
                                              [RandGRUCell, hidden_size, hidden_size, self.bias, self.random_bias,
                                               self.kl_bias, sigma_pi, sigma_start])
        elif mode == 'RNN_TANH':
            self.layers = init_stacked_layers(self.num_layers, RandRNNLayer,
                                              [RandRNNCell, input_size, hidden_size, self.bias, 'tanh',
                                               self.random_bias, self.kl_bias, self.sigma_pi, sigma_start],
                                              [RandRNNCell, hidden_size, hidden_size, self.bias, 'tanh',
                                               self.random_bias, self.kl_bias, self.sigma_pi, sigma_start])
        elif mode == 'RNN_RELU':
            self.layers = init_stacked_layers(self.num_layers, RandRNNLayer,
                                              [RandRNNCell, input_size, hidden_size, self.bias, 'relu',
                                               self.random_bias, self.kl_bias, self.sigma_pi, sigma_start],
                                              [RandRNNCell, hidden_size, hidden_size, self.bias, 'relu',
                                               self.random_bias, self.kl_bias, self.sigma_pi, sigma_start])
        else:
            raise ValueError("Unrecognized RNN mode: " + mode)
...
class RandRNN(RandRNNBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(RandRNN, self).__init__(mode, *args, **kwargs)


class RandLSTM(RandRNNBase):
    """
    Examples::
        nn.LSTM
    """
    ...
    def __init__(self, *args, **kwargs):
        super(RandLSTM, self).__init__('LSTM', *args, **kwargs)
